<template>
  <div class="loginForm">
    <input v-model.lazy="userName" placeholder="Username..." /><br />
    <input v-model.lazy="email" placeholder="Email..." /><br />
    <input v-model.lazy="passWord" placeholder="Password" /><br />
...
export default {
  ...
  methods: {
    ...
    logIn() {
      console.log(
        `${this.userName} ${this.email} try to log in with psw: ${
          this.passWord
        }, keep logged: ${this.keepLogIn ? "yes" : "no"}`
      );
      this.$emit("userHasLoggedSuccesful()", this.userName);
    },
  },
};
</script>
...
.loginForm {
  /*width: 40rem;*/
  display: block;
  background-color: #333; /*var(--background_mod);*/
  color: var(--mainText);
...
.loginForm > input[type="checkbox"] {
  width: 0.8rem; /*box-shadow: inset 1em 1em #ddd;*/
}
