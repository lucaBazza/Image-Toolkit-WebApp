<template>
  <div class="catalogDiv">
    <img
      v-if="!catalogIsReady"
      src="@/assets/loading-io-spinner.gif"
      alt="Catalog loading spinner"
      class="isReadySpinner"
    />
...
export default {
    ...
    async deleteAllImages() {
      ...
      if (r == true) {
        ...
        const res = await fetch(`${this.___urlServerImage}/deleteAll`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ utente: this.catalogoDB.catalogOwner }),
        }).catch((err) => {
          console.log(`Server api ${url} is down 😭`);
          catalogIsOffline = true;
        });
  ...
  async mounted() {
    ...
    // RICHIEDE CATALOGO
    const updateDatasCatalog = await this.requestCatalogForUser(
      `${this.___urlServerImage}/imagelist`,
      this.catalogoDB.catalogOwner
    );
    ...
    // TODO VALIDITA
    console.log(
      `TODO check validità database todo ${updateDatasCatalog.catalogName} \t dimensione: ${updateDatasCatalog.immagini.length} 🌁`
    );
